...
// DingTalk groups have openConversationIds that aren't human-readable. Users
// (or a future `dws chat search` sync) can store a friendly name in
// group_aliases — used to label the chatroom in the dashboard.
export function listAliases(): Map<string, string> {
  const rows = db()
    .prepare('SELECT chatroom_id, name FROM group_aliases')
    .all() as Array<{ chatroom_id: string; name: string }>;
  return new Map(rows.map((r) => [r.chatroom_id, r.name]));
}

export function setAlias(chatroomId: string, name: string) {
  db()
    .prepare(
      `INSERT INTO group_aliases (chatroom_id, name, updated_at) VALUES (?, ?, ?)
       ON CONFLICT(chatroom_id) DO UPDATE SET name = excluded.name, updated_at = excluded.updated_at`,
    )
    .run(chatroomId, name, Date.now());
}

export function aliasFor(chatroomId: string): string | null {
  const row = db()
    .prepare('SELECT name FROM group_aliases WHERE chatroom_id = ?')
    .get(chatroomId) as { name: string } | undefined;
