...
function migrate(d: Database.Database) {
  // chatroom_id holds the DingTalk openConversationId for groups (or userId
  // for private chats). message_id holds the DingTalk msgId (text).
  d.exec(`
    ...
    CREATE TABLE IF NOT EXISTS message_links (
      ...
      created_at INTEGER NOT NULL,
      PRIMARY KEY (chatroom_id, message_id, canonical_url)
    );
    ...
    CREATE TABLE IF NOT EXISTS topic_messages (
      ...
      PRIMARY KEY (topic_id, chatroom_id, message_id),
  ...
  ensureColumn(d, 'messages', 'sender_id', 'TEXT');
}
...
const SEED_VERSION = 'dingtalk_v1_2026_05_26';

const DEFAULT_GROUPS: Array<{ name: string; color: string; emoji: string }> = [
  { name: '团队 · 日常', color: '#ef4444', emoji: '💬' },
  { name: '项目冲刺', color: '#22c55e', emoji: '🚀' },
  { name: '产品讨论', color: '#06b6d4', emoji: '🎯' },
  { name: '工程协作', color: '#6366f1', emoji: '💻' },
  { name: '客户对接', color: '#10b981', emoji: '🤝' },
  { name: '运营增长', color: '#f59e0b', emoji: '📈' },
  { name: 'AI · 研究', color: '#a855f7', emoji: '🤖' },
  { name: '设计协同', color: '#ec4899', emoji: '🎨' },
  { name: '管理通知', color: '#0ea5e9', emoji: '📢' },
  { name: '行政 · 后勤', color: '#eab308', emoji: '📋' },
  { name: '兴趣 · 闲聊', color: '#fb7185', emoji: '🏘️' },
];
