...
function cleanContent(s: string): string {
  const text = s
    .replace(/\[图片\]/g, '')
    .replace(/\[(?:文件|视频|语音|表情|位置|名片|钉钉文档卡片)\]/g, '')
    .replace(/\[该消息暂不支持显示\]/g, '')
    .replace(/https?:\/\/\S+/g, ' ')
  ...
  const parts = text
    ...
    .map((part) =>
      part
        .replace(/^\[引用\]\s*/g, '')
        .replace(/^\[(?:链接|链接\/文件|图片|视频|表情|文件)\]\s*/g, '')
        .replace(/\s+/g, ' ')
        .trim(),
    )
...
function isPlaceholderTitle(value: string): boolean {
  const text = value
    ...
    .trim();
  if (!text) return true;
  return /^(?:该消息暂不支持显示|钉钉文档卡片)$/i.test(text);
}

function loadCandidateMessages(date: string): SourceMsg[] {
  const rows = db()
    .prepare(
      `SELECT chatroom_id, message_id, sender, content, time, timestamp
       FROM messages
       WHERE date = ?
         AND type NOT IN ('image', 'voice', 'video', 'audio', 'file', 'sticker')
         AND length(content) >= ?
...
function sourceId(m: SourceMsg): string {
  return `${m.chatroom_id}#${m.message_id}`;
}
...
function runCodexJson<T>(prompt: string, timeoutMs = CODEX_TIMEOUT_MS): Promise<T> {
  return new Promise((resolve, reject) => {
    const dir = mkdtempSync(join(tmpdir(), 'dingtalk-topics-'));
...
function buildExtractionPrompt(
  ...
  return `你是钉钉群「话题雷达」的聚合引擎。请直接用 LLM 判断语义相关性，找出 ${date} 的主要讨论话题。
...
- 遇到「[该消息暂不支持显示]」「[钉钉文档卡片]」等占位文案不要当成话题标题或摘要。
- 遇到链接/视频/卡片占位时，只能根据前后文里真正有人讨论的对象命名；没有可读上下文就丢弃。
...
只输出严格 JSON，格式：
{"topics":[{"title":"...","summary":"...","message_ids":["群id#message_id"]}]}
...
- 删除过泛、重复、证据不足的话题。
- 删除钉钉占位卡片、资源封面、头像链接、无语义数字串等占位话题。
...
export async function buildTopicsForDate(
  ...
  const sessions = await dwsSessions(readConfig().trackedGroups ?? []).catch(() => []);
  ...
  for (let i = 0; i < valid.length; i++) {
    ...
    const insertMsg = db().prepare(
      'INSERT OR IGNORE INTO topic_messages (topic_id, chatroom_id, message_id, score) VALUES (?, ?, ?, ?)',
    );
    ...
    const tx = db().transaction(() => {
      ...
      for (let index = 0; index < c.members.length; index++) {
        ...
        insertMsg.run(tid, member.chatroom_id, member.message_id, 1 - index / 1000);
        savedMessages++;
      }
    });
    tx();
...
export async function getTopicDetail(id: number): Promise<TopicDetail | null> {
  ...
  const rows = db()
    .prepare(
      `SELECT m.chatroom_id, m.message_id, m.sender, m.content, m.time, m.timestamp, m.type, tm.score
       FROM topic_messages tm
       JOIN messages m ON m.chatroom_id = tm.chatroom_id AND m.message_id = tm.message_id
       WHERE tm.topic_id = ?
