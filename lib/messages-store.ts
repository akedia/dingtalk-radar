...
export interface MessageRow extends DwsMessage {
  chatroom_id: string;
  date: string;
}
...
export function dateOfMessage(m: DwsMessage): string {
...
export function bulkInsertMessages(chatroomId: string, messages: DwsMessage[]): number {
  ...
  `);
  let inserted = 0;
  const tx = db().transaction((msgs: DwsMessage[]) => {
    for (const m of msgs) {
