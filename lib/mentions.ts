...
import { dwsHistory } from './dws';
import type { DwsMessage } from './dws-types';
    ...
    db()
      .prepare(
        `INSERT OR IGNORE INTO mentions
          (chatroom_id, message_id, sender, content, time, timestamp, seen)
         SELECT chatroom_id, message_id, sender, content, time, timestamp, 0
         FROM messages
         WHERE ${predicate.sql}`,
      )
      .run(...predicate.params);
...
export async function scanMentions(
  ...
  let messages: DwsMessage[] = [];
  try {
    messages = await dwsHistory(chatroomId, since, until, 5000);
  } catch {
    return 0;
  }
  ...
  const upsert = db().prepare(`
    INSERT OR REPLACE INTO mentions
      (chatroom_id, message_id, sender, content, time, timestamp, seen)
    VALUES (?, ?, ?, ?, ?, ?, COALESCE((SELECT seen FROM mentions WHERE chatroom_id = ? AND message_id = ?), 0))
  `);

  let inserted = 0;
  const insert = db().transaction((items: DwsMessage[]) => {
...
export function listMentions(limit = 100): MentionRow[] {
  ...
  return db()
    .prepare(
      'SELECT chatroom_id, message_id, sender, content, time, timestamp, seen FROM mentions ORDER BY timestamp DESC LIMIT ?',
    )
