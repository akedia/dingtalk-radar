                ...
                <div className="mt-3 space-y-2">
                  {report.followups.length === 0 ? (
                    ...
                    report.followups.slice(0, 8).map((m) => (
                      <div key={m.message_id} className="rounded-md px-2 py-2 text-[12px] leading-5 text-[var(--text-2)] hover:bg-[var(--surface-2)]">
...
function clusterTopics(messages: ReportMessage[]): TopicReport[] {
  ...
  for (const m of messages) {
    if (assigned.has(m.message_id)) continue;
...
const MEANINGLESS_TITLE_RE =
  /^(?:\[?链接\]?\s*)?(?:该消息暂不支持显示|钉钉文档卡片)[。.]?$/i;

const NOISE_URL_HOSTS = new Set([
  'static.dingtalk.com',
  'static-legacy.dingtalk.com',
  'img.alicdn.com',
]);
...
function meaningfulMessageText(content: string): string {
  const withoutUrls = stripUrls(content)
    .replace(/&amp;/g, '&')
    .replace(/\[(?:链接|链接\/文件|钉钉文档卡片)\]\s*该消息暂不支持显示[。.]?/gi, ' ')
    .replace(/\[该消息暂不支持显示\][。.]?/gi, ' ')
