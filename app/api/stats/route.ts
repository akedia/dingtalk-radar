...
async function loadSessionsSafe(limit: number): Promise<DwsSession[]> {
  const cfg = readConfig();
  if (cfg.demoMode) return listLocalSessionsFallback(limit);
  const cached = cache.get(CK.sessions()) as DwsSession[] | undefined;
  try {
    const stubs = await dwsSessions(cfg.trackedGroups ?? []);
    const known = listLocalSessionsFallback(limit);
    const byId = new Map<string, DwsSession>();
    for (const s of stubs) byId.set(s.username, s);
    for (const s of known) {
      const existing = byId.get(s.username);
      byId.set(s.username, existing ? { ...existing, ...s, is_group: true } : s);
    }
    const merged = Array.from(byId.values());
    cache.set(CK.sessions(), merged, 60);
    return merged;
  } catch (e) {
    if (cached?.length) return cached;
    console.warn('dws sessions failed, falling back to local radar.db', e);
