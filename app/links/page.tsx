    ...
    <section className="card flex min-h-0 min-w-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {loading ? (
          ...
          <div className="py-16 text-center text-[11px] text-[var(--text-3)]">当天没有解析到文章 / 文档链接</div>
        ) : (
          <div className="space-y-1.5">
            {items.map((item) => (
              <div key={`${item.chatroom_id}:${item.message_id}:${item.canonical_url}`} className="rounded-md border border-transparent px-2 py-2 hover:border-[var(--border-soft)] hover:bg-[var(--surface-2)]">
...
function sourceLabel(source?: string): string {
  if (source === 'dingtalk_card') return '群卡片消息';
...
function sourceClass(source?: string): string {
  ...
  if (source === 'dingtalk_card') return `${base} bg-[var(--accent-soft)] text-[var(--accent)]`;
